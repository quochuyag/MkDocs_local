#!/usr/bin/env bash
# 01-vagrant-up.sh — Spin up 4 VMs Ubuntu 22.04 cho lab Async/Semi-sync.
...
# Ép Vagrant dùng VirtualBox; tránh dính Hyper-V/WSL2 provider khi chạy từ WSL bash.
...
for vm in "${VMS[@]}"; do
  ...
  bring_up "$vm" 2 || {
    ...
    log "       Gợi ý: tăng RAM host, đóng app khác, hoặc 'vagrant halt nodeX' các VM khác rồi 'vagrant up ${vm}' riêng."
    exit 1
  }
done

log "==> Trigger provisioner ssh-trust (cần cho 08-failover-test.sh: manual-failover.sh ssh node3 → CHANGE REPLICATION SOURCE)"
vagrant provision --provision-with ssh-trust 2>&1 | tee -a "${LOG}" \
  || log "    WARNING: ssh-trust provisioner gặp lỗi — 08-failover-test có thể fail step reconfigure node3"
...
log "==> Bước 1 hoàn tất. Tiếp theo: bash demo/01-async-semisync/02-prepare-os.sh"
