#!/usr/bin/env bash
# 01-vagrant-up.sh — Spin up 4 VMs Ubuntu 22.04 cho Vagrant Lab (sẵn sàng cho mọi giải pháp HA).
...
if ! command -v VBoxManage >/dev/null; then
  for P in "/c/Program Files/Oracle/VirtualBox" "/c/Program Files (x86)/Oracle/VirtualBox"; do
    if [[ -x "${P}/VBoxManage.exe" ]]; then
      export PATH="${P}:${PATH}"
      alias VBoxManage="${P}/VBoxManage.exe" 2>/dev/null || true
      break
    fi
  done
fi
...
log "==> Bước 1 hoàn tất. Tiếp theo: bash demo/08-vagrant-lab/02-prepare-os.sh"
